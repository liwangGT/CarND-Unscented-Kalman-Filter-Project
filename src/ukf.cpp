...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  Xsig_aug.block<7,7>(0,1) = xM + sqrt(lambda_aug_ + n_aug_)*A;
  Xsig_aug.block<7,7>(0,8) = xM - sqrt(lambda_aug_ + n_aug_)*A;
  ...
  //set weights
  weights_aug_(0) = lambda_aug_/(lambda_aug_+n_aug_);
  weights_aug_.tail(2*n_aug_).setConstant(0.5/(lambda_aug_+n_aug_));
  //predict state mean
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  MatrixXd Tc = MatrixXd(n_aug_, n_z);
  Tc.setZero();
